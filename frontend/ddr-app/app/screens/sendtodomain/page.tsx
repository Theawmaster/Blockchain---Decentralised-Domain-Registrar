...
export default function SendToDomainPage() {
  ...
  const { sendTransactionAsync } = useSendTransaction(); // send transaction fn

  const { data: balanceData } = useBalance({ address }); // fetch wallet balance
  const balance = Number(balanceData?.formatted || "0"); // balance in ETH

  const [domain, setDomain] = useState(""); // domain name input
  const [amount, setAmount] = useState(""); // amount to send
  const [resolved, setResolved] = useState<`0x${string}` | null>(null); // resolved address
